...
namespace Book.DA.SQLServer
{
    ...
    public partial class ProduceMaterialdetailsAccessor : EntityAccessor, IProduceMaterialdetailsAccessor
    {
        ...
        public double SelectMaterialQty(string productid, DateTime dateStart, DateTime dateEnd, string workHouseId, string invoiceXOIds)
        {
            string sql = "select sum(isnull(Materialprocessum,0)) as Materialprocessum from ProduceMaterialdetails pmd left join ProduceMaterial pm on pm.ProduceMaterialID=pmd.ProduceMaterialID where pmd.ProductId='" + productid + "' and pm.ProduceMaterialDate between '" + dateStart + "' and '" + dateEnd + "' and pm.WorkHouseId='" + workHouseId + "' and pm.InvoiceXOId in (" + invoiceXOIds + ")";
