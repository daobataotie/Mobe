...
namespace Book.DA.SQLServer
{
    ...
    public partial class StockAccessor : EntityAccessor, IStockAccessor
    {
        ...
        public IList<Model.StockSeach> SelectOutAndInDepot(DateTime startDate, DateTime endDate, string startProductCategory, string endProductCategory, string depotId, string handBookId)
        {
            ...
            string produceOut = "select '生产退料单' as InvoiceType,pd.ProduceMaterialExitId  as InvoiceNo,(select InvoiceId from InvoiceXO where CustomerInvoiceXOId=pe.CustomerInvoiceXOId) as XOId,pe.ProduceExitMaterialDate as InvoiceDate,pd.ProductId,p.Id as PID,p.ProductName,pd.ProduceQuantity as InvoiceQuantity,pd.DepotPositionId,p.ProductCategoryId,p.ProductCategoryId2,p.ProductCategoryId3,pd.PronoteHeaderId ,pd.HandbookId,pd.HandbookProductId from ProduceMaterialExitDetail pd left join ProduceMaterialExit pe on pd.ProduceMaterialExitId=pe.ProduceMaterialExitId left join Product p on pd.ProductId=p.ProductId where pd.ProduceQuantity<>0 and pd.ProduceQuantity is not null and pe.ProduceExitMaterialDate between '" + startDate + "' and '" + endDate + "'";
