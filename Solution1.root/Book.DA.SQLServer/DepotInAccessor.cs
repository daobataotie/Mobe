...
namespace Book.DA.SQLServer
{
    ...
    public partial class DepotInAccessor : EntityAccessor, IDepotInAccessor
    {
        ...
        public IList<Model.DepotIn> SelectByDateAndOther(DateTime startDate, DateTime endDate, Model.Product product, string depotInId, Model.Employee employee, Model.Employee employee0, string depotId, Model.Supplier supplier)
        {
            ...
            sql.Append("SELECT d.DepotInId,d.DepotInDate,d.Description,(SELECT EmployeeName FROM Employee WHERE Employee.EmployeeId=d.EmployeeId) AS EmployeeId,(SELECT EmployeeName FROM Employee WHERE Employee.EmployeeId=d.Employee0Id) AS Employee0Id,(SELECT DepotName FROM Depot WHERE Depot.DepotId=d.DepotId) AS DepotId,(SELECT SupplierShortName FROM Supplier WHERE SupplierId=d.SupplierId) AS SupplierId,(select ProductName from Product p where p.ProductId=did.ProductId) as ProductName,did.DepotInQuantity FROM depotIn d right join DepotInDetail did on d.DepotInId=did.DepotInId WHERE DepotInDate BETWEEN '" + startDate + "' AND '" + endDate + "'");
            if (product != null)
