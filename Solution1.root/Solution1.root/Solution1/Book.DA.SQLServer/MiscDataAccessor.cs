...
namespace Book.DA.SQLServer
{
    ...
    public partial class MiscDataAccessor : Accessor, IMiscDataAccessor
    {
        ...
        public System.Data.DataTable SelectByCondition(string queryId, string depotId, string depotPositionId, string productCategoryId)
        {
            ...
            {
                case "Q14":
                    sql = "select (select DepotName from Depot where DepotId = (select DepotId from depotposition where depotpositionId = stock.depotpositionId)) as DepotName,(select productName from product where productid = stock.productid) productName,(select ProductDescription from product where productid = stock.productid) ProductDescription,(select id from product where productid = stock.productid) spid,(select CnName from ProductUnit INNER JOIN product ON product.DepotUnitId = ProductUnit.ProductUnitId and product.productid = stock.productid) CnName,(select Id from depotposition where depotpositionId = stock.depotpositionId)  as DepotPositionId,StockQuantity1 as Quantity from stock WHERE (depotid='" + depotId + "'  OR '" + depotId + "'='null' OR '" + depotId + "'='') AND (DepotPositionId ='" + depotPositionId + "' OR '" + depotPositionId + "'='null' OR '" + depotPositionId + "'='') AND (productid IN(SELECT ProductId FROM Product WHERE ProductCategoryId='" + productCategoryId + "')  OR '" + productCategoryId + "'='null' OR '" + productCategoryId + "'='')";
                    break;
            }
            if (!string.IsNullOrEmpty(sql))
            {
        ...
        {
            string sql = "select *, isnull((InvoiceXSDetailQuantity+InvoiceXTDetailQuantity),0) "
                + "as InvoiceDetailQuantity, isnull((InvoiceXSDetailMoney1+InvoiceXTDetailMoney1),0) "
                + "as InvoiceDetailMoney from (select ProductId,ProductName,ProductBaseUnit,ProductSpecification"
                + ", (select isnull(sum(invoicexsdetailquantity),0) from invoicexsdetail where productid = "
                + "product.productid and invoiceid in (select invoiceid from invoicexs where invoiceDate "
                + "between @start and @end)) as InvoiceXSDetailQuantity"
                + ",(select isnull(-sum(invoicextdetailquantity),0) from invoicextdetail where "
                + "productid = product.productid and invoiceid in (select invoiceid from invoicext where "
                + "invoiceDate between @start and @end)) as InvoiceXTDetailQuantity"
                + ",(select isnull(sum(InvoiceXSDetailMoney1),0) from invoicexsdetail where "
                + "productid = product.productid and invoiceid in (select invoiceid from invoicexs "
                + "where invoicedate between @start and @end)) as InvoiceXSDetailMoney1"
                + ", (select isnull(-sum(InvoiceXTDetailMoney1),0) from invoicextdetail where "
                + "productid = product.productid and invoiceId in (select Invoiceid from invoicext "
                + "where invoicedate between @start and @end)) as InvoiceXTDetailMoney1 "
                + "from product where product.productid between @startid and @endid) a "
