//------------------------------------------------------------------------------
//
// 说明：该文件中的内容是由代码生成器自动生成的，请勿手工修改！
//
// file name：CustomerPicturesManager.autogenerated.cs
// author: peidun
// create date：2009-10-26 下午 05:56:40
//
//------------------------------------------------------------------------------
using System;
using System.Collections.Generic;

namespace Book.BL
{
    public partial class CustomerPicturesManager
    {
		///<summary>
		/// Data accessor of dbo.CustomerPictures
		///</summary>
		private static readonly DA.ICustomerPicturesAccessor accessor = (DA.ICustomerPicturesAccessor)Accessors.Get("CustomerPicturesAccessor");
		
		/// <summary>
		/// Select by primary key.
		/// </summary>		
		public Model.CustomerPictures Get(string picturesId)
		{
			return accessor.Get(picturesId);
		}
		
		public bool HasRows(string picturesId)
		{
			return accessor.HasRows(picturesId);
		}
		
		public bool HasRows()
		{
			return accessor.HasRows();
		}
		
		public bool Exists(string id)
		{
			return accessor.Exists(id);
		}
		
		public Model.CustomerPictures GetById(string id)
		{
			return accessor.GetById(id);
		}
		
		public bool ExistsExcept(Model.CustomerPictures e)
		{
			return accessor.ExistsExcept(e);
		}
		public bool HasRowsBefore(Model.CustomerPictures e)
		{
			return accessor.HasRowsBefore(e);
		}
		
		public bool HasRowsAfter(Model.CustomerPictures e)
		{
			return accessor.HasRowsAfter(e);
		}
		
		public Model.CustomerPictures GetFirst()
		{
			return accessor.GetFirst();
		}
		
		public Model.CustomerPictures GetLast()
		{
			return accessor.GetLast();
		}
		
		public Model.CustomerPictures GetPrev(Model.CustomerPictures e)
		{
			return accessor.GetPrev(e);
		}
		
		public Model.CustomerPictures GetNext(Model.CustomerPictures e)
		{
			return accessor.GetNext(e);
		}
		/// <summary>
		/// Select all.
		/// </summary>
		public IList<Model.CustomerPictures> Select()
		{
			return accessor.Select();
		}
		
		/// <summary>
		/// 获取记录总数
		/// </summary>
		public int Count()
		{
			return accessor.Count();
		}
		
		/// <summary>
		/// 获取指定状态、指定分页，并按指定要求排序的记录
		/// </summary>
		public IList<Model.CustomerPictures> Select(Helper.OrderDescription orderDescription, Helper.PagingDescription pagingDescription)
		{
			return accessor.Select(orderDescription, pagingDescription);
		}
		
    }
}
