//------------------------------------------------------------------------------
//
// 说明：该文件中的内容是由代码生成器自动生成的，请勿手工修改！
//
// file name：AcbeginbillReceivableManager.autogenerated.cs
// author: mayanjun
// create date：2011-6-9 16:24:48
//
//------------------------------------------------------------------------------
using System;
using System.Collections.Generic;

namespace Book.BL
{
    public partial class AcbeginbillReceivableManager
    {
		///<summary>
		/// Data accessor of dbo.AcbeginbillReceivable
		///</summary>
		private static readonly DA.IAcbeginbillReceivableAccessor accessor = (DA.IAcbeginbillReceivableAccessor)Accessors.Get("AcbeginbillReceivableAccessor");
		
		/// <summary>
		/// Select by primary key.
		/// </summary>		
		public Model.AcbeginbillReceivable Get(string acbeginbillReceivableId)
		{
			return accessor.Get(acbeginbillReceivableId);
		}
		
		public bool HasRows(string acbeginbillReceivableId)
		{
			return accessor.HasRows(acbeginbillReceivableId);
		}
		
		public bool HasRows()
		{
			return accessor.HasRows();
		}
		
		
		public bool HasRowsBefore(Model.AcbeginbillReceivable e)
		{
			return accessor.HasRowsBefore(e);
		}
		
		public bool HasRowsAfter(Model.AcbeginbillReceivable e)
		{
			return accessor.HasRowsAfter(e);
		}
		
		public Model.AcbeginbillReceivable GetFirst()
		{
			return accessor.GetFirst();
		}
		
		public Model.AcbeginbillReceivable GetLast()
		{
			return accessor.GetLast();
		}
		
		public Model.AcbeginbillReceivable GetPrev(Model.AcbeginbillReceivable e)
		{
			return accessor.GetPrev(e);
		}
		
		public Model.AcbeginbillReceivable GetNext(Model.AcbeginbillReceivable e)
		{
			return accessor.GetNext(e);
		}
		/// <summary>
		/// Select all.
		/// </summary>
		public IList<Model.AcbeginbillReceivable> Select()
		{
			return accessor.Select();
		}
		
		/// <summary>
		/// 获取记录总数
		/// </summary>
		public int Count()
		{
			return accessor.Count();
		}
		
		/// <summary>
		/// 获取指定状态、指定分页，并按指定要求排序的记录
		/// </summary>
		public IList<Model.AcbeginbillReceivable> Select(Helper.OrderDescription orderDescription, Helper.PagingDescription pagingDescription)
		{
			return accessor.Select(orderDescription, pagingDescription);
		}
		public bool ExistsPrimary(string id)
		{
		    return accessor.ExistsPrimary(id);	
	    }
		
    }
}
