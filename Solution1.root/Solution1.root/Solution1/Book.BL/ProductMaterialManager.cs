    ...
    public partial class ProductMaterialManager
    {
        ...
        private void validate(Book.Model.ProductMaterial productMaterial)
        {
            if (string.IsNullOrEmpty(productMaterial.Id))
                throw new Helper.RequireValueException(Model.ProductMaterial.PROPERTY_ID);
            if (IsExistId(productMaterial))
                throw new Helper.InvalidValueException(Model.ProductMaterial.PROPERTY_ID);
            if (string.IsNullOrEmpty(productMaterial.ProductMaterialName))
                throw new Helper.RequireValueException(Model.ProductMaterial.PROPERTY_PRODUCTMATERIALNAME);
            if (IsExistProductMaterialName(productMaterial))
                throw new Helper.InvalidValueException(Model.ProductMaterial.PROPERTY_PRODUCTMATERIALNAME);
        }
