//------------------------------------------------------------------------------
//
// 说明：该文件中的内容是由代码生成器自动生成的，请勿手工修改！
//
// file name：ProductOnlineCheckDetailManager.autogenerated.cs
// author: mayanjun
// create date：2013-3-26 10:59:39
//
//------------------------------------------------------------------------------
using System;
using System.Collections.Generic;

namespace Book.BL
{
    public partial class ProductOnlineCheckDetailManager
    {
		///<summary>
		/// Data accessor of dbo.ProductOnlineCheckDetail
		///</summary>
		private static readonly DA.IProductOnlineCheckDetailAccessor accessor = (DA.IProductOnlineCheckDetailAccessor)Accessors.Get("ProductOnlineCheckDetailAccessor");
		
		/// <summary>
		/// Select by primary key.
		/// </summary>		
		public Model.ProductOnlineCheckDetail Get(string productOnlineCheckDetailId)
		{
			return accessor.Get(productOnlineCheckDetailId);
		}
		
		public bool HasRows(string productOnlineCheckDetailId)
		{
			return accessor.HasRows(productOnlineCheckDetailId);
		}
		
		public bool HasRows()
		{
			return accessor.HasRows();
		}
		
		
		public bool HasRowsBefore(Model.ProductOnlineCheckDetail e)
		{
			return accessor.HasRowsBefore(e);
		}
		
		public bool HasRowsAfter(Model.ProductOnlineCheckDetail e)
		{
			return accessor.HasRowsAfter(e);
		}
		
		public Model.ProductOnlineCheckDetail GetFirst()
		{
			return accessor.GetFirst();
		}
		
		public Model.ProductOnlineCheckDetail GetLast()
		{
			return accessor.GetLast();
		}
		
		public Model.ProductOnlineCheckDetail GetPrev(Model.ProductOnlineCheckDetail e)
		{
			return accessor.GetPrev(e);
		}
		
		public Model.ProductOnlineCheckDetail GetNext(Model.ProductOnlineCheckDetail e)
		{
			return accessor.GetNext(e);
		}
		/// <summary>
		/// Select all.
		/// </summary>
		public IList<Model.ProductOnlineCheckDetail> Select()
		{
			return accessor.Select();
		}
		
		/// <summary>
		/// 获取记录总数
		/// </summary>
		public int Count()
		{
			return accessor.Count();
		}
		
		/// <summary>
		/// 获取指定状态、指定分页，并按指定要求排序的记录
		/// </summary>
		public IList<Model.ProductOnlineCheckDetail> Select(Helper.OrderDescription orderDescription, Helper.PagingDescription pagingDescription)
		{
			return accessor.Select(orderDescription, pagingDescription);
		}
		public bool ExistsPrimary(string id)
		{
		    return accessor.ExistsPrimary(id);	
	    }
		
    }
}
