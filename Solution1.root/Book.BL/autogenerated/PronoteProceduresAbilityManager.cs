//------------------------------------------------------------------------------
//
// 说明：该文件中的内容是由代码生成器自动生成的，请勿手工修改！
//
// file name：PronoteProceduresAbilityManager.autogenerated.cs
// author: mayanjun
// create date：2010-9-23 16:33:52
//
//------------------------------------------------------------------------------
using System;
using System.Collections.Generic;

namespace Book.BL
{
    public partial class PronoteProceduresAbilityManager
    {
		///<summary>
		/// Data accessor of dbo.PronoteProceduresAbility
		///</summary>
		private static readonly DA.IPronoteProceduresAbilityAccessor accessor = (DA.IPronoteProceduresAbilityAccessor)Accessors.Get("PronoteProceduresAbilityAccessor");
		
		/// <summary>
		/// Select by primary key.
		/// </summary>		
		public Model.PronoteProceduresAbility Get(string pronoteProceduresAbilityId)
		{
			return accessor.Get(pronoteProceduresAbilityId);
		}
		
		public bool HasRows(string pronoteProceduresAbilityId)
		{
			return accessor.HasRows(pronoteProceduresAbilityId);
		}
		
		public bool HasRows()
		{
			return accessor.HasRows();
		}
		
		
		public bool HasRowsBefore(Model.PronoteProceduresAbility e)
		{
			return accessor.HasRowsBefore(e);
		}
		
		public bool HasRowsAfter(Model.PronoteProceduresAbility e)
		{
			return accessor.HasRowsAfter(e);
		}
		
		public Model.PronoteProceduresAbility GetFirst()
		{
			return accessor.GetFirst();
		}
		
		public Model.PronoteProceduresAbility GetLast()
		{
			return accessor.GetLast();
		}
		
		public Model.PronoteProceduresAbility GetPrev(Model.PronoteProceduresAbility e)
		{
			return accessor.GetPrev(e);
		}
		
		public Model.PronoteProceduresAbility GetNext(Model.PronoteProceduresAbility e)
		{
			return accessor.GetNext(e);
		}
		/// <summary>
		/// Select all.
		/// </summary>
		public IList<Model.PronoteProceduresAbility> Select()
		{
			return accessor.Select();
		}
		
		/// <summary>
		/// 获取记录总数
		/// </summary>
		public int Count()
		{
			return accessor.Count();
		}
		
		/// <summary>
		/// 获取指定状态、指定分页，并按指定要求排序的记录
		/// </summary>
		public IList<Model.PronoteProceduresAbility> Select(Helper.OrderDescription orderDescription, Helper.PagingDescription pagingDescription)
		{
			return accessor.Select(orderDescription, pagingDescription);
		}
		public bool ExistsPrimary(string id)
		{
		    return accessor.ExistsPrimary(id);	
	    }
		
    }
}
